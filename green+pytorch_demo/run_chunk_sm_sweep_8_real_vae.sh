...
OUTPUT_DIR="${OUTPUT_DIR:-${SCRIPT_DIR}/logs/chunk0_6_sm72_128_real_vae}"
RERUN_COMPLETED="${RERUN_COMPLETED:-0}"

mkdir -p "${OUTPUT_DIR}"

SUMMARY_FILE="${OUTPUT_DIR}/run_status.tsv"
printf 'chunk_idx\tdit_sms\tvae_mode\tstatus\texit_code\tlog_file\n' \
    > "${SUMMARY_FILE}"
    ...
    for dit_sms in $(seq 72 8 128); do
        ...
        if [[ "${RERUN_COMPLETED}" != "1" ]] \
            ...
            printf '%s\t%s\t%s\tskipped\t0\t%s\n' \
                "${chunk_idx}" "${dit_sms}" "${vae_mode}" "${log_file}" \
                >> "${SUMMARY_FILE}"
        ...
        echo "============================================================"
        echo "chunk=${chunk_idx} dit_sms=${dit_sms} vae=${vae_mode}"
        echo "log=${log_file}"
        echo "============================================================"
        ...
        printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
            "${chunk_idx}" "${dit_sms}" "${vae_mode}" "${status}" \
            "${exit_code}" "${log_file}" >> "${SUMMARY_FILE}"
    done
done
